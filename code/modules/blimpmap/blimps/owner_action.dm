...
/datum/action/ship_owner/ui_data(mob/user)
	...
	for(var/datum/job/job as anything in parent_ship.job_slots)
		job_increase_allowed[job.title] = allow_job_slot_increase(job)
...
/datum/action/ship_owner/ui_act(action, list/params)
	...
	switch(action)
		...
		if("adjustJobSlot")
			...
			if(!target_job || !COOLDOWN_FINISHED(parent_ship, job_slot_adjustment_cooldown))
				return TRUE
			...
			if(change_amount > 0 && !allow_job_slot_increase(target_job))
				if(!user.client.holder)
					...
					return TRUE
				message_admins("[key_name_admin(user)] has increased the number of slots for [target_job.title] on [parent_ship.name] by [change_amount].")
