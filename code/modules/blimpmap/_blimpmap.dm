 ...
 * # Blimpmap datum
 *
 * This is the base (parent) class for all objects present on the blimpmap
 *
 */
/datum/overmap
	/// Name of the datum, propapagated to its token (blimp, area, docking port e.t.c.)
	var/name
	/// A brief description, should fit in a mouse hover tooltip
	var/desc
	/// Extra info that would fit into a text popup
	var/extra_desc
	/// "Class" which is used instead of its name when the object is obscured
	var/object_class = "MOTHERFUCKING BLIMP! AND IT IS MAGICAL, TOO!!!"
	/// Color of the object on the blimpmap
	var/default_color
	/// Icon state of the token on the blimpmap
	var/token_icon_state = "signal_none"

	var/char_rep

	/// The x position of this datum on the blimpmap. Use [/datum/overmap/proc/move] to change this.
	VAR_FINAL/x
	/// The y position of this datum on the blimpamp. Use [/datum/overmap/proc/move] to change this.
	VAR_FINAL/y
